...
- FIXED: Prevents duplicate nominations by tracking per-day
"""
...
def run_nominalization_scheduled(client):
    ...
    Prevents double nominations by tracking the last nomination date.
    """
    if not MEETING_CHANNEL_ID:
        logger.error("MEETING_CHANNEL_ID not set")
        return
    ...
    # Check if nomination already happened today
    if data.get("last_nomination_date") == today_date:
        logger.info(f"[NOMINATION] Already nominated today ({today_date}) - skipping to prevent duplicates")
        return
    ...
    try:
        ...
        create_nomination(client, MEETING_CHANNEL_ID, selected_id, data, is_auto=True)
        # Track that we nominated today
        data["last_nomination_date"] = today_date
        save_data(data)
...
def run_scheduler(client):
    ...
    DUPLICATE PROTECTION: Nominations are prevented from running twice per day via last_nomination_date tracking.
    """
    ...
    if day_name in ["Wednesday"]:
        if hour_minute >= "05:00":
            ...
            # Only run nominalization if we're past 05:01
...
if __name__ == "__main__":
    logger.info("=== MEETING LEADER BOT - CONTROL PANEL EDITION STARTING (FIXED) ===")
    ...
    logger.info("Bot is now running with control panel and ephemeral features (DUPLICATE NOMINATIONS PREVENTED)")
